...
const GrupoArticuloEdit = () => {
  ...
  const getData = async () => {
    try {
      const respModel = await getModelById('grupo-articulos', id);
      setGrupo(respModel.data);
      console.log('RESPUESTA DE GRUPO=>=', respModel);

    } catch (error) {
      console.log('Error en busqueda de grupo', error);
      setErrorMsg('No se pudieron encontrar los grupos')
    }
  }
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault();        
    try {
      const resp = await updateModel('grupo-articulos', grupo);
      if(resp.status === 200){
      ...
      setErrorMsg('Error, no se pudo actualizar el grupo')
    }
  }
  ...
  return <AdminDashboard title={'Editar Grupo de Artículo'} preImageRoute='../../'>
...
export default GrupoArticuloEdit