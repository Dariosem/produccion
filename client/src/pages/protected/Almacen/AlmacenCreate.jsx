...
const CreateAlmacen = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const resp = await createModel('almacenes', grupo);
      if(resp.status === 200){
        setGrupo(initialGroup);
        navigate(-1)
      } else {
        console.log('RESPUESTA DE ALMACENES CON PROBLEMAS', resp);
      }
    } catch (error) {
      console.log('Error al crear el Almacen::>', error);
      setErrorMsg('Error, no se pudo crear el Almacen')
    }
  ...
  return <AdminDashboard title={'Crear Almacén'} preImageRoute='../'>
